...
from scripts.handlers.folder_management_handler import clean_directory as clean_directory_handler
...
async def clean_directory(
    ...
    try:
        ...
        response = await clean_directory_handler(current_folder_id=folder_id,
                                                 current_user_id=current_user.id,
                                                 db=db)
        if response[CommonConstants.status] == CommonConstants.success:
            return response
        else:
            raise HTTPException(status_code=500, detail=f"Error cleaning directory")
    except Exception as e:
        db.rollback()
